...
import com.lovushkina.repository.FeedbackRepository;
import com.lovushkina.repository.UserRepository;
import com.lovushkina.service.FeedbackService;
...
public class FeedbackServiceImpl implements FeedbackService {
    ...
    public void update(Integer id, Feedback entity) {
        Feedback feedback = feedbackRepository.findById(id).orElseThrow(() ->new DataNotFoundException("Feedback", id));
    ...
    public void update(Integer feedbackId, Feedback entity, Integer userId, Integer appId) {
        Feedback feedback = feedbackRepository.findById(feedbackId).orElseThrow(() ->new DataNotFoundException("Feedback", feedbackId));
        User user = userRepository.findById(userId).orElseThrow(() ->new DataNotFoundException("User", userId));
        App app = appRepository.findById(appId).orElseThrow(() ->new DataNotFoundException("App", appId));
