...
public class DownloadServiceImpl implements DownloadService {
    ...
    public void update(Integer id, Download entity) {
        Download download = downloadRepository.findById(id).orElseThrow(() ->new DataNotFoundException("Download", id));
    ...
    public void update(Integer downloadId, Download entity, Integer userId) {
        User user = userRepository.findById(userId).orElseThrow(() ->new DataNotFoundException("AppCategory", downloadId));
        Download download = downloadRepository.findById(downloadId).orElseThrow(() ->new DataNotFoundException("Download", userId));
        download.setAmount(entity.getAmount());
        download.setTotalPrice(entity.getTotalPrice());
        download.setUser(user);
        downloadRepository.save(download);
